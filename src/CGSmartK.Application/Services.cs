...
public sealed class KnowledgeService(IDocumentRepository documents, IIngestionQueue queue, IKnowledgeIndex index, IngestionOptions options) : IKnowledgeService
{
    public async Task<KnowledgeDocument> UploadAsync(UploadDocumentCommand c, CancellationToken ct)
    { var (hash,replay)=await PdfValidator.ValidateAsync(c.FileName,c.ContentType,c.Length,c.Content,options.MaxPdfBytes,ct); await using(replay) { var existing=await documents.FindByHashAsync(hash,ct); if(existing is not null) return existing; var id=Guid.NewGuid(); var doc=new KnowledgeDocument { Id=id, OriginalFileName=PdfValidator.SafeName(c.FileName), BlobName=$"documents/{id:N}.pdf", Title=c.Title.Trim(), Category=c.Category.Trim(), Access=new(c.Classification,c.AllowedGroups), ContentHash=hash, SizeBytes=c.Length, UploaderId=c.UploaderId }; await documents.SaveAsync(doc,replay,ct); await queue.EnqueueAsync(id,ct); return doc; } }
